...
>
                ...
                return (
                  ...
                  >
                    ...
                    <button
                      ...
                      className="absolute bottom-2 right-2 rounded-full border border-[var(--accent-primary)] bg-[var(--bg-card)] px-3 py-1 text-[10px] uppercase tracking-[0.2em] text-[var(--text-primary)] opacity-0 shadow-lg backdrop-blur-md transition group-hover:opacity-100 hover:bg-[var(--accent-primary)] hover:text-[var(--bg-dark)]"
                    >
                      Zoom
                    </button>
                  </div>
      ...
      {isLightboxOpen && activeImage ? (
        <div
          className="fixed inset-0 z-[9999] flex flex-col bg-[var(--bg-darker)]"
          onClick={() => setIsLightboxOpen(false)}
        >
          {/* Fixed header - always visible */}
          <div
            className="shrink-0 bg-[var(--bg-darker)] px-4 pt-4 md:px-6 md:pt-5"
            onClick={(event) => event.stopPropagation()}
          >
            <div className="mx-auto flex max-w-7xl items-start justify-between gap-3 rounded-[18px] border border-[var(--border-light)] bg-[var(--bg-card)] px-3 py-3 sm:rounded-[20px] sm:px-4">
              ...
              <div className="flex shrink-0 items-center gap-3">
                <div className="rounded-full border border-[var(--border-light)] bg-[var(--bg-dark)] px-3 py-2 text-[10px] uppercase tracking-[0.16em] text-[var(--text-muted)] sm:px-4 sm:text-xs sm:tracking-[0.2em]">
                  {activeIndex + 1} / {images.length}
                </div>
          ...
          {/* Image area */}
          <div
            className="flex min-h-0 flex-1 flex-col bg-[var(--bg-darker)] px-4 pb-4 pt-3 md:px-6 md:pb-5 sm:pt-4"
            onClick={(event) => event.stopPropagation()}
          >
            <div className="mx-auto flex w-full max-w-7xl flex-1 flex-col min-h-0">
              <div className="relative flex min-h-0 flex-1 items-center justify-center overflow-hidden rounded-[28px] border border-[var(--border-light)] bg-[var(--bg-dark)] shadow-[0_25px_100px_rgba(0,0,0,0.25)]">
                ...
                {images.length > 1 && (
                  <>
                    <button
                      type="button"
                      onClick={goPrev}
                      className="absolute left-2 top-1/2 z-10 -translate-y-1/2 rounded-full border border-[var(--accent-primary)] bg-[var(--bg-card)] px-3 py-2 text-xs font-semibold text-[var(--text-primary)] transition hover:bg-[var(--accent-primary)] hover:text-[var(--bg-darker)] sm:left-3 sm:px-4 sm:py-3 sm:text-sm md:left-4"
                    ...
                    <button
                      type="button"
                      onClick={goNext}
                      className="absolute right-2 top-1/2 z-10 -translate-y-1/2 rounded-full border border-[var(--accent-primary)] bg-[var(--bg-card)] px-3 py-2 text-xs font-semibold text-[var(--text-primary)] transition hover:bg-[var(--accent-primary)] hover:text-[var(--bg-darker)] sm:right-3 sm:px-4 sm:py-3 sm:text-sm md:right-4"
